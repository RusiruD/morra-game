...
public class Morra {
  ...
  public void newGame(Difficulty difficulty, int pointsToWin, String[] options) {
    ...
    if(difficulty.name().equals("EASY")){difficultylevel=1;}
    else if(difficulty.name().equals("MEDIUM")){difficultylevel=2;}
    else if(difficulty.name().equals("HARD")){difficultylevel=3;}
    else if(difficulty.name().equals("MASTER")){difficultylevel=4;}
  ...
  public void play() {
    ...
    //if a valid amount of fingers and sum is guessed by the user output the info message.
    if(fingers>=1 && fingers <=5 && sum>=1 && sum<=10){
      ...
      //if a invalid amount of fingers and sum is guessed by the user output an error message.
      else{
      ...
      //depending on the difficulty level, call the appropriate method from the difficulty interface
  ...
  //if the sum of the fingers guessed by the user plus the fingers guessed by the ai is only equal to the sum guessed by the AI, the AI wins
  if((AiGuesses.get(1)== (fingers + AiGuesses.get(0))) && AiGuesses.get(1)!=sum){
    MessageCli.PRINT_OUTCOME_ROUND.printMessage("AI_WINS");

    //if the AI wins the round and has the required number of points to win, print the end game message and set gameLoaded to 0
 ...
 //if the sum of the fingers guessed by the user plus the fingers guessed by the ai is only equal to the sum guessed by the user the user wins
 else if((sum== (fingers + AiGuesses.get(0))) && sum!=AiGuesses.get(1)){
    MessageCli.PRINT_OUTCOME_ROUND.printMessage("HUMAN_WINS");

    //if the user wins the round and has the required number of points to win, print the end game message and set gameLoaded to 0
    ...
    //if neither player guesses the correct sum or they both do the round is a draw, no points awarded
MessageCli.PRINT_OUTCOME_ROUND.printMessage("DRAW");
  ...
  public void showStats() {
    ...
    //print the number of rounds played, the number of points the user has and the number of points the AI has and how many rounds they need to win
    MessageCli.PRINT_PLAYER_WINS.printMessage(name, Integer.toString(humanPoints),Integer.toString(pointsTarget-humanPoints));
    MessageCli.PRINT_PLAYER_WINS.printMessage("Jarvis", Integer.toString(aiPoints),Integer.toString(pointsTarget-aiPoints));
